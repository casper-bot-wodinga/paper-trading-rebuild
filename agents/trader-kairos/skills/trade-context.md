...
### 1. Portfolio State
```
curl -s localhost:5000/portfolio?account=kairos
```
Returns: cash, equity, buying_power, positions, daily_pnl, drawdown

### 2. Current Positions
```
curl -s localhost:5000/positions?account=kairos
```
...
### 5. Sentiment
```
curl -s localhost:5000/sentiment?symbols=KO,F,INTC,PFE
```
Returns: aggregated sentiment scores for your watchlist

### 6. Fear & Greed
```
curl -s localhost:5000/fear_greed
```
Returns: Fear & Greed Index — for FearContrarian strategy
...
If you're in a hurry, `tick_prompt.py` already assembles this context for you. The data above is the same data injected into every tick — you can also call these endpoints directly for mid-tick refreshes.

## Self-Stats

```bash
curl -s localhost:5000/self-stats?account=kairos
```
Returns: win rate, avg PnL per trade, drawdown, regime performance split
