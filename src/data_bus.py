...
# Paths for bar parquet files
_BARS_PATHS = [
    Path.home() / ".openclaw" / "workspace-coder" / "paper-trading-teams" / "shared" / "cache",
    SRC_DIR.parent / "shared" / "cache",
]


def _find_bars_dir() -> Optional[Path]:
    """Find the first existing bars directory."""
    for base in _BARS_PATHS:
        for sub in ["bars", "daily_bars"]:
            p = base / sub
...
def _load_bars_parquet(bars_dir: Path, ticker: str) -> Optional[list]:
    """Load bars from a single ticker parquet file, return list of OHLCV dicts."""
    path = bars_dir / f"{ticker.upper()}.parquet"
    if not path.exists():
        return None
    try:
        import pandas as pd
...
@app.route("/bars")
def bars_endpoint():
    """Get historical OHLCV bars from parquet store.
    ...
    Returns:
        {"symbols": {ticker: [bars...], ...}, "count": N, "symbols_found": [...], "source": "parquet"}
    """
    symbols_str = request.args.get("symbols", "")
    ...
    sub_dir = "daily_bars" if interval == "daily" else "bars"
    ...
    if bars_dir is None:
        return jsonify({"error": f"No bars directory found (looked in: {_BARS_PATHS})", "symbols": {}, "count": 0, "symbols_found": [], "source": "none"}), 404
    ...
    for symbol in symbols:
        records = _load_bars_parquet(bars_dir, symbol)
        if records is None:
            continue
    ...
    return jsonify({
        ...
        "source": f"parquet:{bars_dir}",
        "interval": interval,
    })


@app.route('/calendar')
def calendar_endpoin       symbols_found.append(symbol)\n            total_count += len(records)\n\n    return jsonify({\n        \"symbols\": result,\n        \"count\": total_count,\n        \"symbols_found\": symbols_found,\n        \"symbols_requested\": symbols,\n        \"source\": f\"parquet:{bars_dir}\",\n        \"interval\": interval,\n    })\n\n\n@app.route('/calendar')\ndef calendar_endpoint():"
